/**
 * Fuzzer page — public site variant.
 * Removed: runs_seen, runs_analyzed metrics.
 * Added: Issues filed metric.
 */
...
function headerMetrics(dashboard) {
  ...
  const issuesFiled = safeInt(issueActions.created) + safeInt(issueActions.updated);
  return metricGrid([
    ...
    metric('Issues filed', issuesFiled, { note: 'GitHub issues created or updated' }),
  ]);
}
...
export function render(container, dashboard) {
  ...
  const hero = el('header', { class: 'hero' }, [
    el('div', { class: 'hero-row' }, [
      el('div', {}, [
        el('h2', {}, ['Fuzzer']),
        el('p', {}, ['Fuzzer findings and issues filed for Valkey.']),
      ]),
    ]),
  ]);
  ...
  container.replaceChildren(fragment([
    ...
    panel({
      ...
      subtitle: 'Live anomaly mix and issue action breakdown.',
