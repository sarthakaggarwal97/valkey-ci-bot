...
def build_validated_pr_body(
    ...
    *,
    ...
    """Build a PR body for the issue-first, CI-validated pipeline.

    Includes: Fixes #N, root cause, what the fix does, validation
    evidence (link to the CI run showing N clean passes), and the
    number of attempts.
    """
    lines: list[str] = []
    primary_failure = (
    ...
    if issue_number:
        lines.append(f"Fixes #{issue_number}\n")

    lines.append("## Fix Summary\n")
    lines.append(
        f"Automated fix for `{primary_failure}` in `{report.job_name}`.\n"
    )

    lines.append("| Field | Value |")
    lines.append("|---|---|")
    if issue_url:
        lines.append(f"| Tracking issue | [#{issue_number}]({issue_url}) |")
    lines.append(f"| Failing CI run | [workflow run]({workflow_run_url}) |")
    if validation_run_url:
        lines.append(f"| Validation run | [CI validation]({validation_run_url}) |")
    ...
    lines.append(root_cause.description or "_No root cause description._")
    lines.append("")
    if root_cause.rationale:
        lines.append(f"**Rationale:** {root_cause.rationale}\n")

    if root_cause.files_to_change:
        lines.append("### Files Changed\n")
        for f in root_cause.files_to_change:
            lines.append(f"- `{f}`")
        lines.append("")
    ...
    lines.append("---")
    lines.append(
        "*This PR was generated by [valkey-ci-agent]"
        "(https://github.com/sarthakaggarwal97/valkey-ci-agent). "
