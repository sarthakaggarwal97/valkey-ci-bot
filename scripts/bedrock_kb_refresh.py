"""Refresh curated Valkey corpora and Bedrock Knowledge Base data sources."""
...
class RefreshArgs:
    """Arguments required to refresh the Valkey KB setup."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--code-data-source-name", default="valkey-code-custom")
    parser.add_argument("--web-kb-id", default=DEFAULT_WEB_KB_ID)
    parser.add_argument("--web-data-source-id")
    parser.add_argument("--web-data-source-name", default="valkey-docs-web")
    parser.add_argument(
        "--web-seed-url",
        action="append",
        dest="web_seed_urls",
    ...
    parser.add_argument(
        "--missing-only",
        action="store_true",
        help="Only ingest documents not already present in the custom data source. Useful for resuming a partial run.",
    )
...
def stage_curated_corpus(source_root: Path, staging_root: Path, repo_url: str, branch: str, head_sha: str) -> tuple[int, int]:
...
def make_document_text(relative_path: str, source_text: str, repo_slug: str, branch: str, head_sha: str) -> str:
...
def make_inline_attributes(relative_path: str, repo_slug: str, branch: str, head_sha: str) -> list[dict[str, object]]:
    ...
    if path.suffix:
        attrs.append({"key": "extension", "value": {"type": "STRING", "stringValue": path.suffix.lower()}})
    if path.parts:
        attrs.append({"key": "top_level_dir", "value": {"type": "STRING", "stringValue": path.parts[0]}})
    language = infer_language(path)
    if language:
        attrs.append({"key": "language", "value": {"type": "STRING", "stringValue": language}})
    return attrs
...
def build_custom_document(path: Path, root: Path, repo_slug: str, branch: str, head_sha: str) -> CuratedDocument:
    ...
    payload = {
        ...
        "metadata": {
            ...
            "inlineAttributes": make_inline_attributes(relative_path, repo_slug, branch, head_sha),
        },
    }
...
def iter_custom_documents(root: Path, repo_slug: str, branch: str, head_sha: str) -> Iterator[CuratedDocument]:
...
def find_data_source_id(agent_client, knowledge_base_id: str, name: str) -> str | None:
...
def wait_for_data_source(agent_client, knowledge_base_id: str, data_source_id: str) -> None:
    ...
    while True:
        ...
        if status in {"DELETE_UNSUCCESSFUL", "FAILED"}:
            raise RuntimeError(f"Data source {data_source_id} entered terminal status {status}.")
        if time.monotonic() >= deadline:
            raise TimeoutError(f"Timed out waiting for data source {data_source_id} to become AVAILABLE.")
        logger.info("Waiting for data source %s to become AVAILABLE. Current status: %s", data_source_id, status)
        time.sleep(DATA_SOURCE_POLL_INTERVAL_SECONDS)


def is_document_operation_limit_error(exc: ClientError) -> bool:
    """Return whether Bedrock rejected the request due to concurrent direct-ingest limits."""
    error = exc.response.get("Error", {})
    return error.get("Code") == "ValidationException" and (
        "concurrent IngestKnowledgeBaseDocuments and DeleteKnowledgeBaseDocuments requests" in error.get("Message", "")
    )


def is_immutable_web_parsing_error(exc: ClientError) -> bool:
    """Return whether Bedrock refused to mutate an existing web data source parsing config."""
    error = exc.response.get("Error", {})
    return error.get("Code") == "ValidationException" and (
        "vectorIngestionConfiguration.parsingConfiguration cannot be updated once created" in error.get("Message", "")
    )


def call_with_document_retry(operation, description: str):
    """Retry Bedrock direct-ingest document operations when account concurrency is exhausted."""
...
def ensure_code_data_source(agent_client, args: RefreshArgs) -> str:
    """Create or update the custom data source used for source code."""
    data_source_id = find_data_source_id(agent_client, args.code_kb_id, args.code_data_source_name)
    payload = {
...
def ensure_web_data_source(agent_client, args: RefreshArgs) -> str:
    """Create or update the Valkey docs web crawler data source."""
    data_source_id = args.web_data_source_id or find_data_source_id(agent_client, args.web_kb_id, args.web_data_source_name)
    payload = {
...
def list_existing_document_ids(agent_client, knowledge_base_id: str, data_source_id: str) -> set[str]:
...
def delete_stale_documents(agent_client, knowledge_base_id: str, data_source_id: str, document_ids: set[str]) -> int:
...
def ingest_custom_documents(agent_client, knowledge_base_id: str, data_source_id: str, documents: Iterable[CuratedDocument]) -> tuple[int, int]:
    ...
    for batch_number, batch in enumerate(chunk_documents(documents), start=1):
        ...
        indexed += sum(1 for detail in details if detail.get("status") in TERMINAL_DOCUMENT_STATUSES)
        logger.info("Submitted batch %s with %s documents.", batch_number, len(batch))
    return ingested, indexed


def start_ingestion(agent_client, knowledge_base_id: str, data_source_id: str, description: str) -> str:
...
def refresh(args: RefreshArgs) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="valkey-kb-refresh-") as tmp_dir:
        ...
        file_count, total_bytes = stage_curated_corpus(repo_root, staged_root, args.repo_url, args.branch, head_sha)
        logger.info("Prepared curated corpus with %s files (%.2f MiB).", file_count, total_bytes / (1024 * 1024))
    ...
    existing_ids = list_existing_document_ids(agent_client, args.code_kb_id, code_data_source_id)
    desired_ids = {document.document_id for document in documents}
    documents_to_ingest = documents
    if args.missing_only:
        documents_to_ingest = [document for document in documents if document.document_id not in existing_ids]
