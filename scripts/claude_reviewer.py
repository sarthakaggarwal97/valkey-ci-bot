...
def _skeptic_pass(
    ...
    prompt = (
        ...
        f"{diff_text}\n\n"
        f"## Your role\n"
        f"Catch false positives. Keep legitimate concerns. "
        f"**Keep anything with concrete evidence**; drop only clearly speculative or duplicate findings.\n\n"
        f"For each candidate, decide verdict = \"keep\" or \"drop\".\n\n"
        f"### Drop only if\n"
        f"- The claim is clearly speculative — not supported by reading the actual code\n"
        f"- It's a clear duplicate of another finding (same root cause, same file area)\n"
        f"- It's a pure style nit with no correctness, safety, or clarity impact\n"
        f"- A reasonable maintainer would obviously say \"not worth a comment\" — not borderline cases\n\n"
        f"### Keep if\n"
        f"- You can verify by reading code that the concern is real, even if minor\n"
        f"- The concern points to a concrete spot in the code with a concrete impact\n"
        f"- It's a test coverage gap, doc inaccuracy, or missing analog that a reviewer would mention\n"
        f"- You're unsure whether to keep or drop — **keep it**. Prefer keeping a borderline "
        f"finding over dropping a real one.\n\n"
        f"You may downgrade `severity` if the evidence is weaker than the candidate claims, "
        f"but still keep the finding.\n\n"
        f"## Output\n"
        ...
        f'  {{"index": 1, "verdict": "drop", "severity": "low", "reason": "speculative, no concrete trigger in code"}}\n'
        f"]\n"
        f"```\n\n"
...
def review_pr(
    ...
    *,
    ...
    prompt = (
        ...
        f"{valkey_block}\n\n"
        f"## Goal\n"
        f"Produce a broad set of candidate findings. **A separate skeptic pass will filter** "
        f"false positives before anything is posted, so your job here is coverage, not precision. "
        f"Flag anything that looks suspicious; the skeptic will drop speculative/duplicate/style-only items.\n\n"
        f"## What to flag\n"
        ...
        f"If a finding feels 60% likely to be a real issue, flag it. The skeptic pass is the "
        f"quality filter, not this pass.\n\n"
        f"**Avoid only:** pure style/formatting nits (skeptic will catch those too), completely "
        f"speculative concerns with no code-reading basis, duplicates of findings you've already made "
        f"in this response.\n\n"
        f"## Output (IMPORTANT)\n"
        f"You may think out loud at any point, but **your response must end with a valid JSON array**. "
        f"Aim for 3-8 candidate findings on a typical PR; more is fine for complex changes. "
        f"If the PR is truly clean (trivial doc change, pure refactor with identical behavior), "
        f"return `[]` at the end.\n\n"
        f"Line numbers must be in the NEW version of the file (after the PR's changes) and must "
        f"point to lines present in the diff for GitHub to post inline.\n\n"
        f"```json\n"
        f"[\n"
    ...
    try:
        ...
        format_retry_prompt = (
            "Your previous review produced prose analysis instead of a valid JSON array.\n\n"
            "Here was your full analysis:\n"
            "---\n"
            f"{result_text[:8000]}\n"
            "---\n\n"
            "Now extract every concrete concern from the analysis above and return them as a "
            "JSON array. Even concerns you described as 'low severity' or 'probably fine' count — "
            "a separate skeptic pass will filter them. Your job here is extraction, not judgment.\n\n"
            "Rules:\n"
            "- For each concrete code-level concern you raised in the analysis, emit one finding\n"
            "- Use the paths and line numbers you mentioned in your analysis\n"
            "- If your analysis genuinely found no specific code-level concerns, return `[]`\n"
            "- Do NOT return `[]` just because you feel the concerns are minor — that's the skeptic's call\n\n"
            "Return ONLY the JSON array, no prose, no markdown fences.\n\n"
            "Schema:\n"
            "```\n"
            '[{"path": "src/file.c", "line": 42, "severity": "medium", '
            '"title": "Brief", "body": "Short comment", "confidence": "high"}]\n'
            "```"
        )
